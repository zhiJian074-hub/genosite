...
# 这个模块会在训练时读取你的 gLM/batch.pkl.glm.embs.pkl 文件，
# 并且根据蛋白质 entry id 自动匹配对应的 gLM 向量。
class GLMEmbeddingLookup:
    """
    从 gLM embedding 文件加载蛋白质向量。
    """

    def __init__(self, glm_emb_path="gLM/batch.pkl.glm.embs.pkl"):
        with open(glm_emb_path, "rb") as f:
            data = pk.load(f)
            # 自动判断是 list 还是 dict
            if isinstance(data, dict):
                self.glm_embs = data
            elif isinstance(data, list):
                self.glm_embs = {k: v for k, v in data}
            else:
                raise ValueError(f"未知的 gLM embedding 格式: {type(data)}")

        sample_vec = list(self.glm_embs.values())[0]
        self.dim = len(sample_vec)
        print(f"[gLM] 已加载 {len(self.glm_embs)} 条蛋白质 embedding, 每条 {self.dim} 维")

    def __call__(self, entry_ids):
        embs = []
        for eid in entry_ids:
            if eid in self.glm_embs:
                embs.append(torch.tensor(self.glm_embs[eid], dtype=torch.float32))
            else:
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def _tokenization(self, sample, modal, max_length=None):
        ...
        encoded_sample = tokenizer(sample, max_length=max_length, padding='max_length', truncation=True,
                                   return_tensors="pt")
